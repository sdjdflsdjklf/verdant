...
### Fixed
- WikiLink internal links now resolve correctly to published page URLs instead of causing 404 errors
- WikiLinks render as proper Markdown links instead of displaying raw HTML code on published pages
- Link resolution supports multiple formats: file name (`[[ARCHITECTURE]]`), full path (`[[AI_Actions/ARCHITECTURE]]`), and title (`[[Architecture Overview]]`)
- Added `LinkGraphService.resolveWikiLink()` for accurate URL mapping during site generation
...
### Fixed
- Remove insecure `rejectUnauthorized: false` SSL bypass
...
[v1.0.0]: https://github.com/sdjdflsdjklf/verdant/releases/tag/v1.0.0
